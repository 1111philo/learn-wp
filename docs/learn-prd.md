...
**Learn** is a WordPress plugin by 11:11 Philosopher's Group that adds a "Learn" custom post type, a "Courses" taxonomy, and a "Lesson Groups" tag taxonomy. A **super admin** enters a course title, description, and learning objectives into the Learn admin panel on the Multisite network's main site. A seven-agent AI pipeline (powered by the Anthropic Claude API) then generates a cohesive course narrative, structured lesson plans, full lesson content, practice activities with gamification mechanics, and a summative assessment — all saved as WordPress posts authored by a dedicated system agent user and organized under the appropriate Course and Lesson Group taxonomy terms.

The super admin reviews all generated material — lesson plans, individual lessons, activities, the course category, and the course name and description — and can add feedback at any level to trigger regeneration through the agent pipeline. Once satisfied, the admin **publishes** the course, at which point the content becomes permanently immutable. Published courses cannot be updated; the admin must create and generate a new course instead.

The plugin runs on a **WordPress Multisite** network. Learners sign up through a **registration form generated by the plugin**. When a learner signs up, they receive their own WordPress subsite with a **copy** of the default course content. The learner can then add feedback to any content within their own WordPress site, triggering regeneration that applies **to their copy only** — personalizing the learning experience without affecting the source material or other learners. Learners complete activities within their own WordPress site, building real content (pages, posts, even entire sites) as portfolio artifacts. Learners **cannot create new courses** — they select from courses the admin has published, tracked through a learner panel on their subsite. An Activity Assessment Agent evaluates the learner's WordPress content against the generated rubrics and mastery criteria.
...
1. Let a **super admin** create a complete, structured course from three inputs: title, description, and learning objectives — reviewed and published from the Learn admin panel on the network's main site.
...
11. Provide **learner self-registration** — learners sign up through a plugin-generated form, receive their own WordPress subsite with a copy of course content, and can personalize that content through feedback-driven regeneration on their own site.
12. Enforce **publish immutability** — once a course is published, its content cannot be updated. The admin must create a new course to make changes, ensuring learners always have a stable version.
...
- **Signs up through a plugin-generated registration form** — does not need admin enrollment
...
- **Cannot create new courses** — selects from admin-published courses via a learner panel on their subsite
...
Learners sign up through a **registration form generated by the plugin** — there is no admin enrollment step. The plugin handles the entire flow:

1. **Registration form:** The plugin provides a frontend registration page (shortcode or block) where learners enter a username, email, and password.
...
3. **Default course content:** The plugin copies all published course content (lessons, activities, assessments) from the main site to the learner's subsite. This is the learner's personal copy — they can personalize it through feedback without affecting anyone else.
4. **Capabilities:** The learner has the `administrator` role on their own subsite — they can create and manage all content on their site. However, the Learn admin panel (course creation) is **not available** on learner subsites. Learners see a **learner panel** instead, with course selection and progress tracking.
5. **Cross-site access:** The Activity Assessment Agent (running on the main site) reads learner content from subsites using `switch_to_blog()` / `restore_current_blog()` — standard WordPress Multisite API.

#### 4.7.4 Content Copy Model

When a learner signs up or selects a new course, the plugin copies published course content from the main site to the learner's subsite:
...
- The `_1111_source_post_id` and `_1111_source_blog_id` meta fields link each copied post back to its origin on the main site (for telemetry and admin progress tracking)
...
Once a super admin publishes a course, the source content on the main site becomes **permanently immutable** — neither the admin nor the agent pipeline can modify it. This ensures:

- Learners who already have a copy are never out of sync with a silently-updated source
- The admin's review and feedback cycle has a definitive endpoint
- Content versioning is explicit: a new course is a new course, not a stealth update

**To change published content, the admin must create a new course.** The published course remains available to existing learners. The admin can optionally mark an old course as superseded.
...
The review panel is the admin's central place to evaluate all generated material before publishing.

#### 8.2.2 Publish Flow

Once the super admin is satisfied with all course material:

1. Admin clicks **Publish Course** from the course review panel.
...
7. To make changes, the admin must create a new course from scratch.
...
**Admin feedback** operates on draft content on the main site, before publishing. Once published, admin feedback UI is removed.

**Learner feedback** operates on the learner's personal copy of the content on their subsite. Regeneration affects only their copy — other learners and the main site source are unaffected.
...
On learner subsites, the Learn menu shows a **learner panel** instead of the admin course creation UI. The learner panel has:
...
Learners can provide feedback on **their own copy** of course content — the same feedback UI surfaces as the admin (sidebar panels, meta boxes, taxonomy term editors) are available on learner subsites. When a learner submits feedback:
...
- [ ] **Learner panel** on learner subsites (replaces admin course creation UI):
...
- [ ] Test full flow: admin creates course → publishes → learner signs up → selects course → completes activities → gets assessed
...
14. Published courses are immutable — no admin or agent action can modify published content on the main site. The admin must create a new course.
